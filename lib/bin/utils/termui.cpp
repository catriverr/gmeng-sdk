...
static void _udraw_display(Gmeng::t_display& __d) {
    ...
    std::cout << Gmeng::colors[__d.v_outline_color] << "┌" << repeatString("─", __d.v_width+2) << "┐" << Gmeng::resetcolor;
    ...
    for (const auto& dp : __d.v_drawpoints) {
        ...
        std::cout << Gmeng::colors[__d.v_textcolor] << dp << Gmeng::resetcolor; vc_vector_count++;
    };
    _uset_cursor_pos(__vcurr_pos.x, __vcurr_pos.y);
    std::cout << Gmeng::colors[__d.v_outline_color] << "└" << repeatString("─", __d.v_width+2) << "┘" << Gmeng::resetcolor;
};
